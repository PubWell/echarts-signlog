...
import SignLogScale from '../scale/SignLog';
...
export function alignScaleTicks(
    scale: IntervalScale | LogScale | SignLogScale,
    axisModel: AxisBaseModel<Pick<NumericAxisBaseOptionCommon, 'min' | 'max'>>,
    alignToScale: IntervalScale | LogScale | SignLogScale
) {
    ...
    if (scale.type === 'signlog') {
        const logBase = mathLog((scale as SignLogScale).base);
        rawExtent = [mathLog(rawExtent[0]) / logBase, mathLog(rawExtent[1]) / logBase];
    }
